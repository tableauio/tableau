...
func Test_processWhenEmitTimezonesByRegexp(t *testing.T) {
	type args struct {
		jsonStr      string
		locationName string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    `{"time":"2022-01-01T00:00:00Z"}`,
			wantErr: false,
		},
		{
			name: "UTC+8",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := processWhenEmitTimezonesByRegexp(tt.args.jsonStr, tt.args.locationName)
