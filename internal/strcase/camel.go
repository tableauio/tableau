...
// toPascalCase converts a string to PascalCase using a chunk-based
// rule set. The result is STYLE2024-compliant: digits stay attached
// to the adjacent letter run (e.g. "Tier1" -> "Tier1", not "Tier_1"),
// and acronyms are treated as ordinary words.
//
...
func (ctx *Strcase) toPascalCase(s string) string {
	...
	flushChunk := func(end int) {
		...
		out.WriteString(ctx.transformPascalChunk(chunk))
	}
...
// transformPascalChunk applies the per-chunk transformation rules
// described in toPascalCase to a single non-empty chunk that does
...
func (ctx *Strcase) transformPascalChunk(chunk string) string {
	if chunk == "" {
		return chunk
	}
...
// transformPlainChunk applies rules 1-3 from toPascalCase to a chunk
...
// ToPascal converts a string to PascalCase under STYLE2024 rules.
//
// Examples:
//
...
//	"Tier1"               -> "Tier1"
//	"HeroNTagMFcX_SCORE"  -> "HeroNTagMFcXScore"
func (ctx *Strcase) ToPascal(s string) string {
	return ctx.toPascalCase(s)
}

// ToCamel converts a string to camelCase under STYLE2024 rules. It is
// derived from ToPascal by lower-casing the first character of the
// PascalCase result.
//
// Examples:
//
...
func (ctx *Strcase) ToCamel(s string) string {
	return lowerFirst(ctx.toPascalCase(s))
}
