...
type bookExporter struct {
	...
	// (see WorksheetOptions.union_split_threshold). Empty when no union is split.
...
func (x *bookExporter) writeAuxFile(aux *unionAuxFile, path string) error {
	...
	slices.Sort(keys)
	for _, k := range keys {
		header.P(`option `, k, ` = `, x.ProtoFileOptions[k], `;`)
	}
...
// placed in each shard file when WorksheetOptions.union_split_shard_size is
// not set.
...
func (x *sheetExporter) exportUnion() error {
	...
	// The threshold is set per-sheet via the @TABLEAU metasheet's
	// `UnionSplitThreshold` column, which maps to WorksheetOptions here.
	threshold := int(x.ws.GetOptions().GetUnionSplitThreshold())
	...
	for _, field := range x.ws.Fields {
		...
		//
		// NOTE: only rename types that are locally defined in this sheet
		// (i.e. field.FullType is empty, which means it isn't a predefined
		// external type). Predefined types must stay as their original
...
func (x *sheetExporter) exportUnionSplit(subMsgs []unionSubMsg) error {
	shardSize := int(x.ws.GetOptions().GetUnionSplitShardSize())
	if shardSize <= 0 {
	...
	for shardIdx := 0; shardIdx < numShards; shardIdx++ {
		...
		for i, sm := range shardMsgs {
			...
			depth := 1
			// NOTE: we do NOT reuse assignFieldNumbers here because the shard
			// message is a fresh top-level definition; there is no oldMD to
			// diff against (previous shard splits, if any, are transparent
			// to protoc). Field numbers come from the input fields as-is.
			for _, field := range sm.field.Fields {
