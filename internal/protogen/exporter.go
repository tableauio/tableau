...
func (x *bookExporter) export(checkProtoFileConflicts bool) error {
	...
	if checkProtoFileConflicts {
		if err := x.checkOutputConflicts(path); err != nil {
			return err
		}
	}
...
func (x *bookExporter) checkOutputConflicts(mainPath string) error {
...
// writeProtoHeader writes syntax/edition, package, imports, and file options.
func (x *bookExporter) writeProtoHeader(p *printer.Printer, imports *treeset.Set) {
	p.P("// Code generated by tableau (protogen v", Version, "). DO NOT EDIT.")
	p.P("// clang-format off")
	p.P("")
	if x.Edition != "" {
		p.P(`edition = "`, x.Edition, `";`)
	} else {
		p.P(`syntax = "proto3";`)
	}
	p.P("")
	p.P("package ", x.ProtoPackage, ";")
	p.P("")

	for _, key := range imports.Values() {
		p.P(`import "`, key, `";`)
	}
	p.P("")
	...
	slices.Sort(keys)
	for _, k := range keys {
		p.P(`option `, k, ` = `, x.ProtoFileOptions[k], `;`)
	}
}
...
// unionSubMessage is a union oneof variant with a message body.
type unionSubMessage struct {
	field *internalpb.Field
	typ   string // type name without parent prefix
}

func unionSubMessageType(unionName, subType string) string {
	return unionName + subType
}

func collectUnionSubMessages(fields []*internalpb.Field) ([]unionSubMessage, map[string]bool) {
	...
	for _, field := range fields {
		...
		subMessages = append(subMessages, unionSubMessage{field: field, typ: typ})
		shardTypes[typ] = true
	}
	return subMessages, shardTypes
}
...
func (x *sheetExporter) exportUnion() error {
	...
	subMessages, shardTypes := collectUnionSubMessages(x.ws.Fields)
	...
	for _, field := range x.ws.Fields {
		...
		if shouldShard && shardTypes[typ] {
			typ = unionSubMessageType(x.ws.Name, typ)
		}
	...
	if shouldShard {
		if err := x.exportUnionShards(subMessages, shardSize); err != nil {
			return err
		}
	} else {
		x.p.P()
		...
		for _, sm := range subMessages {
			...
			if parentMD != nil {
				oldMD = parentMD.Messages().ByName(protoreflect.Name(sm.typ))
			}
			if err := x.exportUnionSubMessageBody(sm, oldMD, 2); err != nil {
				return err
			}
			x.p.P("  }")
		}
...
// exportUnionShards writes sub-messages as top-level `<Union><SubType>` types
// in shard proto files imported by the main file.
func (x *sheetExporter) exportUnionShards(subMessages []unionSubMessage, shardSize int) error {
	...
	for shardIdx := 0; shardIdx < numShards; shardIdx++ {
		...
		end := min(start+shardSize, len(subMessages))
		...
		for i, sm := range shardMessages {
			...
			shardPrinter.P("message ", unionSubMessageType(x.ws.Name, sm.typ), " {")
			oldMD := tempSe.findUnionSubMessageMD(x.ws.Name, sm.typ)
			if err := tempSe.exportUnionSubMessageBody(sm, oldMD, 1); err != nil {
				return err
			}
			shardPrinter.P("}")
...
func (x *sheetExporter) exportUnionSubMessageBody(sm unionSubMessage, oldMD protoreflect.MessageDescriptor, depth int) error {
	reserved := x.assignFieldNumbers(sm.field.Fields, oldMD)
	...
	for _, field := range sm.field.Fields {
		...
		if err := x.exportField(depth, field, sm.field.Name, oldFD); err != nil {
			return err
		}
	}
	return nil
...
// findUnionSubMessageMD returns the previous descriptor for PreserveFieldNumbers:
// top-level `<Union><SubType>`, then nested `<Union>.<SubType>`.
func (x *sheetExporter) findUnionSubMessageMD(unionName, subType string) protoreflect.MessageDescriptor {
	if md := x.findMDFromGeneratedProtos(unionSubMessageType(unionName, subType)); md != nil {
		return md
	}
...
// worksheetOptionsForProto omits generation-only fields such as union_shard_size.
