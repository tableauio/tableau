...
func (x *bookExporter) export() error {
	...
	p1 := printer.New()
	p1.P(generatedFileHeaderPrefix, " (protogen v", Version, "). DO NOT EDIT.")
	...
	// NOTE: use the workbook path rather than x.wb.GetName(), which is the
	// snake-cased basename shared by the conflicting workbooks, hence useless
	// for telling them apart.
	if err := x.gen.registerGeneratedProtoFile(path, x.wb.GetOptions().GetName()); err != nil {
