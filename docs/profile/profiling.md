...
The largest top-level sample is now the Windows system-call boundary beneath
`storeMessage`: 3.28 CPU-seconds, almost entirely reached while writing
generated files. Go reports Windows system calls through `runtime.cgocall` even
when the toolchain has CGO disabled. This sample is output work and must not be
attributed to sheet parsing.
...
### 5. Reduce generated-file write cost

Priority: P2

The final Windows profile attributes 3.28 sampled CPU-seconds to file writes
under `storeMessage`. Measure two changes independently:

1. Avoid rewriting an existing file when its generated bytes are unchanged.
2. Limit concurrent writes if storage saturation causes scheduler and I/O
   contention.

Keep either change only when an alternating normal-mode benchmark improves the
median. Content comparison must include every output format and preserve file
